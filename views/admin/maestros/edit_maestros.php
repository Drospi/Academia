<?php foreach ($usuarios as $usuario) {  ?>
    <div id="maestro<?php echo $usuario['email'] ?>" class="modal fixed w-full h-full top-0 left-0 flex items-center justify-center hidden">
        ...
        <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
            <div class="modal-content py-4 text-left px-6">
                ...
                <!-- Contenido del modal -->
                <form action="/maestros/edit" id="maestros_form" method="post">
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center border border-gray-300 p-2 rounded">
                            <input type="date" placeholder="Correo" id="born_date" value="<?php echo $usuario['born_date'] ?>" name="born_date" class="w-full focus:outline-none">
                        </div>
                    </div>

                    <div class="mb-4">
                        <label for="rol">Clase Asignada</label>
                        <select class="w-full cursor-pointer appearance-none border border-gray-300 p-2 rounded leading-tight focus:outline-none focus:border-blue-500" name="materia" id="materia">
                            <?php
                            foreach ($materias as $materia) {
                                $selected = ($materia['id_maestro'] == $usuario['id']) ? 'selected' : '';
                            ?>
                                <option value="<?php echo $materia['id']; ?>" <?php echo $selected; ?>><?php echo $materia['materia']; ?></option>
                            <?php }?>
                        </select>
                    </div>
...
<script>
    <?php foreach ($usuarios as $usuario) {  ?>
        document.getElementById('maestro<?php echo $usuario['id'] ?>').addEventListener('click', function() {
            document.getElementById('maestro<?php echo $usuario['email'] ?>').classList.remove('hidden');
        ...
        document.querySelectorAll('.closeModal').forEach(function(close) {
            close.addEventListener('click', function(e) {
                document.getElementById('maestro<?php echo $usuario['email'] ?>').classList.add('hidden');
                e.preventDefault();
            });
        })
